...
namespace PublicApiAnalyzer
{
    ...
    internal static class AnalyzerExtensions
    {
        ...
        private static Tuple<WeakReference<Compilation>, StrongBox<SourceText>> settingsCache
            = Tuple.Create(new WeakReference<Compilation>(null), default(StrongBox<SourceText>));
        ...
        /// <param name="context">The analysis context.</param>
        /// <param name="action">Action to be executed at completion of parsing of a document.</param>
        public static void RegisterSyntaxTreeActionHonorExclusions(this CompilationStartAnalysisContext context, Action<SyntaxTreeAnalysisContext, SourceText> action)
        {
            Compilation compilation = context.Compilation;
            ConcurrentDictionary<SyntaxTree, bool> cache = GetOrCreateGeneratedDocumentCache(compilation);
            StrongBox<SourceText> settingsCache = GetOrCreatePublicApiCache(compilation);
            ...
            context.RegisterSyntaxTreeAction(
                c =>
                {
                    ...
                    SourceText settings = settingsCache.Value;
                    if (settings == null)
                    {
                        SourceText updatedSettings = SettingsHelper.GetPublicApi(c.Options, c.CancellationToken);
                        SourceText previous = Interlocked.CompareExchange(ref settingsCache.Value, updatedSettings, null);
        ...
        /// <summary>
        /// Gets a <see cref="StrongBox{T}"/> which can store a <see cref="SourceText"/> instance to improve
        /// efficiency across multiple analyzers which examine settings.
        /// </summary>
        /// <param name="compilation">The compilation which the cache applies to.</param>
        /// <returns>A <see cref="StrongBox{T}"/> which can store a <see cref="SourceText"/> instance.</returns>
        public static StrongBox<SourceText> GetOrCreatePublicApiCache(this Compilation compilation)
        {
            ...
            {
                var replacementCache = Tuple.Create(new WeakReference<Compilation>(compilation), new StrongBox<SourceText>(null));
                while (true)
                {
        ...
        /// <typeparam name="TLanguageKindEnum">Enum type giving the syntax node kinds of the source language for which
        /// the action applies.</typeparam>
        public static void RegisterSyntaxNodeActionHonorExclusions<TLanguageKindEnum>(this CompilationStartAnalysisContext context, Action<SyntaxNodeAnalysisContext, SourceText> action, TLanguageKindEnum syntaxKind)
            where TLanguageKindEnum : struct
        {
            context.RegisterSyntaxNodeActionHonorExclusions(action, LanguageKindArrays<TLanguageKindEnum>.GetOrCreateArray(syntaxKind));
        }
        ...
        /// <typeparam name="TLanguageKindEnum">Enum type giving the syntax node kinds of the source language for which
        /// the action applies.</typeparam>
        public static void RegisterSyntaxNodeActionHonorExclusions<TLanguageKindEnum>(this CompilationStartAnalysisContext context, Action<SyntaxNodeAnalysisContext, SourceText> action, ImmutableArray<TLanguageKindEnum> syntaxKinds)
            where TLanguageKindEnum : struct
        {
            Compilation compilation = context.Compilation;
            ConcurrentDictionary<SyntaxTree, bool> cache = GetOrCreateGeneratedDocumentCache(compilation);
